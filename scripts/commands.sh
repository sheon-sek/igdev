...
cmd_self_test() {
  ...
  grep -Fxq '      context: .' "$ROOT_DIR/docker-compose.yml" || die 'Docker build context must be repository root'
...
EOF_MODULE_TEST
    ...
    [[ "$module_test_info" == "com.example.devctl-test"$'\t'"Devctl Test"$'\t'"1.0.0" ]] || die 'Private .modl metadata parsing is invalid'
  fi
...
cmd_gateway() {
 ...
 "$ROOT_DIR/docker-compose.yml" || die 'Docker build context must be repository root'
  grep -Fq 'dockerfile: docker/ignition/Dockerfile' "$ROOT_DIR/docker-compose.yml" || die 'Compose Dockerfile path is invalid'
  grep -Fq 'COPY .runtime/docker/modules/' "$ROOT_DIR/docker/ignition/Dockerfile" || die 'Dockerfile is not consuming runtime module staging'
  log 'Docker module staging boundary OK'
  local catalog
