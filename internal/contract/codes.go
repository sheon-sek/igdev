...
var CodeDocs = []CodeDoc{
	...
	{CodeModuleArtifactMissing, "1", "An enabled module is neither built into the image nor staged as a `.modl`, or a `[modules].artifacts` glob matched nothing.",
		"Stage the module with `igdev module add <file.modl>`, or fix the glob so it matches what the build produced."},
