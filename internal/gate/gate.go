...
// and against a fixture. The enforcement entry point is Require; `init` and
// `setup` are the repair paths and deliberately do not call it, so its first
// enforcing consumer arrives with the pipeline commands (ticket 11). Ticket 08
// exercises Evaluate here and reports its verdict through `igdev status`.
package gate
...
// Version — together with the Instance identity, the ports allocated to it, and
// the moment it was first set up.
//
// The stamp fields alone decide whether the checkout is current; the Instance
// fields are carried state that survives a re-setup (a refresh keeps the
// instance_id, its namespace, its ports, and created_at).
type Stamp struct {
	// Schema is this record's own format version.
	Schema int `json:"schema"`
	// InstanceID is the Instance's random UUID, minted at first setup. It is
	// never derived from a path and never regenerated by a re-setup (ADR 0003).
	...
	// Ports are the loopback ports allocated to this Instance. They are recorded
	// here, in the disposable Checkout Setup, and never in the tracked contract.
	Ports ports.Triplet `json:"ports"`
	// CreatedAt is when the Instance was first set up, RFC 3339 UTC. A later
	// setup keeps it: it dates the Instance, not the record.
	CreatedAt string `json:"created_at"`
}

// Namespace is the Instance's Docker namespace, igdev-<short-id>, or "" when the
// record carries no instance identity.
func (s Stamp) Namespace() string {
	if s.InstanceID == "" {
		return ""
	}
	return instance.Namespace(s.InstanceID)
}

// Decode reads a Checkout Setup record. It reports false when the record is
// absent or unreadable, the same verdict the Gate gives such a checkout.
func Decode(raw []byte) (Stamp, bool) {
	if len(raw) == 0 {
		return Stamp{}, false
	}
	var stamp Stamp
...
func stampState(in Input, digest string, schema int) StampState {
	...
	if in.SetupReadError != "" {
		return StampStale
	}
	stamp, ok := Decode(in.SetupRaw)
	if !ok {
		return StampStale
	}
