...
- [`README.zh-CN.md`](README.zh-CN.md) — the Chinese onboarding guide (it points here
  and at the generated English reference).

---

# Appendix: the legacy `devctl` foundation

Everything below documents the vendored bash foundation this repository grew from
(`./devctl`, `scripts/`, `config/`, `docker/`, `.env`, `.runtime/`). It still works and
is still exercised by the legacy `foundation-ci` workflow; igdev replaces it. The legacy
tree, its workflows, and this appendix are retired by ticket #19 — until then the two
sit side by side.
